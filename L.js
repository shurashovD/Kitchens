...
const Directions = {'right':'Первая', 'left':'Вторая'};
let COORDS = [];
let MATRIX = [];
let displayCoords = [];
let nonImpactPoints = [];
...
const saveInputData = (point, type, value) => {
    ...
    // непоследняя точка;
        ...
        // сначала модули;
        if (initialType.classList.contains('modul-before')) {
            const pointNum = Letters.indexOf(point);
...
const addInternalPoints = () => {
    ...
    if ( initialBaseSideSelect.classList.contains('first-side') ) {
        if (initialBaseSideOrient.classList.contains('vertical')) {
            ...
            for ( let i=leftAngleNum; i<initialTopInput.value; i++ ) {
            ...
            y = MATRIX[Letters[initialTopInput.value-1]].y;
            updatePointInMatrix(Letters[length-3], x, y);
        }
    }
    else {
        if (initialBaseSideOrient.classList.contains('vertical')) {
            let x = MATRIX[Letters[initialTopInput.value-1]].x - parseFloat(secondWidthWorkTop);
            ...
            let y = MATRIX[Letters[initialTopInput.value-1]].y - parseFloat(secondWidthWorkTop);
            updatePointInMatrix(Letters[length-3], x, y);
...
const displaySize = () => {
    ...
    const firstSide = initialBaseSideSelect.classList.contains('first-side');
    const trait = 2;
    const indent = 25;
...
initialTopInput.addEventListener('input', (event) => {
    let flag = false;
    for (let point in COORDS) {
    ...
    if ( Letters.indexOf(initialLeftAngle.textContent) >= (initialTopInput.value-1) )
    initialLeftAngle.textContent = Letters[initialTopInput.value-2];
    if ( Letters.indexOf() )
    baseSideLabel();
});
initialTopInput.addEventListener('change', () => {
    if ( initialTopInput.value<3 ) initialTopInput.value = 3;
    initialBaseSideSelect.classList.add('first-side');
    if ( Letters.indexOf(initialLeftAngle.textContent) >= (initialTopInput.value-1) )
    initialLeftAngle.textContent = Letters[initialTopInput.value-2];
    baseSideLabel();
...
initialLeftAngle.addEventListener('click', () => {
    let j = Letters.indexOf(initialLeftAngle.textContent) + 1;
    if ( j == 0 ) j = 1;
    if ( j>=(initialTopInput.value-1) ) j = 1;
    initialLeftAngle.textContent = Letters[j];
    baseSideLabel();
});
...
// надпись на базовой стороне;
const baseSideLabel = () => {
    initialBaseSideSelect.textContent = initialLeftAngle.textContent + Letters[initialTopInput.value-1];
    if ( initialBaseSideSelect.classList.contains('first-side') )
...
initialNext.addEventListener('click', () => {
    ...
    if ( flag ) {
        alert('Нет точек для определения первой столешницы.');
        const event = new Event('hashchange');
        location.hash = '#initial';
        window.dispatchEvent(event);
    }
    ...
    if ( length != 0 ) return;
    for (let i=0; i<+initialTopInput.value+3; i++) {
