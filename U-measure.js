...
//let initialBaseSide = 'Fird';                  // базовая столешница;
let initialLeftAngle = null;                    // угловая точка;
let initialRightAngle = null;                   // угловая точка;
...
// обновить чертеж;
const renderDraw = () => {
    if ( initialBaseSide == 'First' ) {
        initialBasePointOne = 'A';
        initialBasePointTwo = initialLeftAngle;
    }
    if ( initialBaseSide == 'Second' ) {
        initialBasePointOne = initialLeftAngle;
        initialBasePointTwo = initialRightAngle;
    }
    if ( initialBaseSide == 'Fird' ) {
        initialBasePointOne = initialRightAngle;
    ...
    if ( checkCross() ) {
        measuringDialog.textContent = 'Fehler.';
        measuringDialog.title = 'Ошибка построения эскиза. Грани пересекаются.';
        console.log(MATRIX);
        return false;
    }
...
const structData = () => {
    ...
    if ( length == 0 ) {
        for (let i=0; i<MeasPoints.size+4; i++) {
...
const addInternalPoints = () => {
    ...
    for (let i=0; i<Letters.length; i++) {
        if ( Letters[i] == initialLeftAngle ) leftAngleNum = i;
        if ( Letters[i] == initialRightAngle ) rightAngleNum = i;
    }
    let length = 0;
    for ( let pointKey in MATRIX ) length++;
    ...
    if ( initialBaseSide == 'First' ) {
        if ( initialBaseSideOrientVertical ) {
            ...
            for ( let i=rightAngleNum; i<MeasPoints.size; i++ ) {
                if ( nonImpactPoints.includes(Letters[i]) ) continue;
            ...
            x = MATRIX[Letters[MeasPoints.size-1]].x;
            updatePointInMatrix(Letters[length-4], x, y);
        }
    }
    else {
        if ( initialBaseSideOrientVertical ) {
            let x = MATRIX[Letters[MeasPoints.size-1]].x - parseFloat(firdWidthWorkTop);
            ...
            let y = MATRIX[Letters[MeasPoints.size-1]].y + parseFloat(firdWidthWorkTop);
            updatePointInMatrix(Letters[length-4], x, y);
...
measuringCanvas.addEventListener('mousedown', (mousedown) => {
    ...
    if (Step == 0) {
        ...
        if ( !initialRightAngle ) {
            if ( (Math.abs(clickX - 940) < 10)&&(Math.abs(clickY - 40) < 10) ) {
                pointByCoords(940, 40, '#ff0000');
                MeasPoints.set(
                    lettersMap.get(MeasPoints.size), {modul: 0, move: 0, angle: 0, x: 940, y: 40}
                );
                initialRightAngle = lettersMap.get(MeasPoints.size-1);
                return;
            }
        }
